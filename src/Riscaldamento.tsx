...
const warmupNotes = [
  {
    ...
    url: 'https://darebee.com/workouts/quick-warmup-workout.html'
  },
  {
...
export default function Riscaldamento() {
      ...
      <ul className="space-y-2">
        {warmupNotes.map((note, index) => (
          <li key={index} className="bg-white dark:bg-gray-800 shadow rounded-lg p-4 text-sm">
            🟢 {note.url ? (
              <a href={note.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 underline">
                {note.label}
              </a>
            ) : (
              note.label
