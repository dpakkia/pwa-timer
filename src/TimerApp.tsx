...
export default function TimerApp() {
  ...
  return (
    <div
      className="w-full flex flex-col items-center text-center select-none bg-black text-white min-h-screen"
    ...
    >
      ...
      {PHASES[phaseIndex].label === 'Break' && (
        <div className="mt-6 space-y-4 w-full max-w-md px-4">
          ...
          {BREAK_WORKOUT.map((exercise) => (
            <div key={exercise.name} className="bg-neutral-900 shadow rounded-lg overflow-hidden">
              <div className="p-4">
                <h3 className="font-medium text-white mb-1">{exercise.name}</h3>
                <p className="text-sm text-[#aaa] mb-2">{exercise.duration}</p>
      ...
      {menuVisible && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300">
          <div
            className="bg-neutral-900 text-white rounded-xl shadow-lg p-6 space-y-4 w-[90%] max-w-xs text-center"
